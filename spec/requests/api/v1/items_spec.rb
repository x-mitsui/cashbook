...
RSpec.describe "Items", type: :request do
  describe "获取账目" do
    ...
    it "按时间筛选" do
      ...
      get "/api/v1/items?happened_after=2018-01-01&happened_before=2018-01-03",
    ...
    it "按时间筛选（边界条件）" do
      ...
      get "/api/v1/items?happened_after=2018-01-01&happened_before=2018-01-02",
    ...
    it "按时间筛选（边界条件2）" do
      ...
      get "/api/v1/items?happened_after=2018-01-01",
        headers: item1.user.get_auth_header
      expect(response).to have_http_status 200
      json = JSON.parse(response.body)
      expect(json["resources"].size).to eq 1
    ...
    it "按时间筛选（边界条件3）" do
      ...
      get "/api/v1/items?happened_before=2018-01-02",
        headers: user1.get_auth_header
      expect(response).to have_http_status 200
      json = JSON.parse(response.body)
      expect(json["resources"].size).to eq 1
    ...
    it "按 kind 筛选" do
      ...
      get "/api/v1/items?kind=income", headers: user.get_auth_header
      expect(response).to have_http_status 200
      json = JSON.parse(response.body)
      expect(json["resources"].size).to eq 1
      expect(json["resources"][0]["amount"]).to eq 200
    end
    it "按 kind 和 page 筛选" do
      user = create :user
      create :item, kind: "income", amount: 200, user: user
      create :item, kind: "expenses", amount: 100, user: user

      get "/api/v1/items?kind=income&page=1", headers: user.get_auth_header
      expect(response).to have_http_status 200
      json = JSON.parse(response.body)
      expect(json["resources"].size).to eq 1
      expect(json["resources"][0]["amount"]).to eq 200
    ...
    it "登录后创建" do
      ...
      expect {
        post "/api/v1/items", params: { amount: 99, tag_ids: [tag1.id, tag2.id],
                                        happened_at: "2018-01-01T00:00:00+08:00", kind: "income" },
  ...
  describe "获取余额" do
    ...
    it "登录" do
      ...
      get "/api/v1/items/balance?happened_after=2018-03-02T15:00:00.000Z&happened_before=2018-03-02T17:00:00.000Z", headers: user.get_auth_header
      expect(response).to have_http_status 200
      json = JSON.parse(response.body)
